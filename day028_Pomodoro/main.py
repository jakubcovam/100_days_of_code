...
def count_down(count):
    ...
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count-1)  # 1000 ... millisecond
    else:
        start_timer()
...
label_check = Label(fg=GREEN, bg=YELLOW, font=(FONT_NAME, 30, "bold"))
label_check.grid(column=1, row=3)

# Button
button_start = Button(text="Start", fg="black", bg="white", font=(FONT_NAME, 10, "bold"),
                      command=start_timer)
button_start.grid(column=0, row=2)
button_reset = Button(text="Reset", fg="black", bg="white", font=(FONT_NAME, 10, "bold"),
                      command=reset_timer)
